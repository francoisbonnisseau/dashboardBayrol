...
import {
  ...
  LoadingState,
  PageTabs as Tabs,
  PageTabPanel as TabsContent,
  PageTabList as TabsList,
  PageTab as TabsTrigger,
...
export default function Analysis() {
  ...
  const [selectionMethod, setSelectionMethod] =
    useState<DataSelectionMethod>('date-range');
  ...
  const fetchFilteredConversations = useCallback(async () => {
    ...
    try {
      ...
      switch (selectionMethod) {
        ...
        case 'sentiment':
          filteredRows = formattedRows.filter((row) =>
            row.sentiment
              .toLowerCase()
              .includes(selectedSentiment.toLowerCase()),
          );
          break;
      ...
      toast.success(
        `Found ${filteredRows.length} conversations matching your criteria`,
      );
    } catch (error) {
  ...
  > => {
    ...
    for (const conversation of selectedConversations) {
      try {
        ...
        console.error(
          `Error fetching messages for conversation ${conversation.conversationId}:`,
          error,
        );
      }
    }
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const conversationsText = conversationsWithMessages
        .map((conv) => {
      ...
      if (googleAI_response) {
        ...
        // Last resort: stringify the whole response to see its structure
          ...
          textContent =
            'Could not extract text content from API response. Please check console for details.';
        }
        ...
        if (textContent) {
          ...
          console.error(
            'Response structure:',
            JSON.stringify(googleAI_response, null, 2),
          );
  ...
  return (
    <>
      <PageHeader
        title="AI Analysis"
        description="Select conversations, configure instructions, then run analysis"
      />
      {/* Header Card */}
      <FilterBar>
      ...
      <div className="flex items-center justify-center py-2">
        <div className="flex items-center w-full max-w-2xl">
          <div
            className={`flex-1 flex flex-col items-center gap-2 ${step === 'select-data' ? 'text-primary' : 'text-muted-foreground'}`}
          >
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${step === 'select-data' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}
            >
              1
            </div>
          ...
          <div
            className={`flex-1 h-0.5 ${step !== 'select-data' ? 'bg-primary' : 'bg-muted'}`}
          />
          <div
            className={`flex-1 flex flex-col items-center gap-2 ${step === 'enter-prompt' ? 'text-primary' : 'text-muted-foreground'}`}
          >
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${step === 'enter-prompt' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}
            >
              2
            </div>
          ...
          <div
            className={`flex-1 h-0.5 ${step === 'view-results' ? 'bg-primary' : 'bg-muted'}`}
          />
          <div
            className={`flex-1 flex flex-col items-center gap-2 ${step === 'view-results' ? 'text-primary' : 'text-muted-foreground'}`}
          >
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${step === 'view-results' ? 'bg-primary text-primary-foreground' : 'bg-muted'}`}
            >
              3
            </div>
      ...
      {step === 'select-data' && (
        <Section
          title="Select Conversations for Analysis"
          description={
            <>Choose a method to filter conversations for AI analysis</>
          }
        >
          <div className="space-y-4">
            <Tabs
              ...
              onValueChange={(value) =>
                setSelectionMethod(value as DataSelectionMethod)
              }
              className="w-full"
            >
              <TabsList aria-label="Conversation selection method">
              ...
              <TabsContent value="sentiment" className="space-y-4">
                <div className="flex flex-col gap-1.5 max-w-xs">
                  ...
                  >
                    <SelectTrigger aria-label="Sentiment" className="h-9">
                      <SelectValue placeholder="Select sentiment" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="very negative">
                        Very Negative
                      </SelectItem>
                      <SelectItem value="negative">Negative</SelectItem>
                      <SelectItem value="neutral">Neutral</SelectItem>
                      <SelectItem value="positive">Positive</SelectItem>
                      <SelectItem value="very positive">
                        Very Positive
                      </SelectItem>
                    </SelectContent>
                  </Select>
            ...
            {selectedConversations.length > 0 && (
              ...
              >
                <Table>
                  ...
                  <TableBody>
                    {selectedConversations.slice(0, 20).map((conversation) => (
                      <TableRow key={conversation.id}>
                        ...
                        <TableCell>{conversation.sentiment}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
      ...
      {step === 'enter-prompt' && (
        <Section
          title="Enter AI Analysis Prompt"
          description={
            <>
              Provide instructions for what you want the AI to analyze in these
              conversations
        ...
        >
          <div className="space-y-4">
            <EditorSurface title="System context">
              <Label className="sr-only" htmlFor="analysis-system-prompt">
              ...
              />
              <p className="text-sm text-muted-foreground">
                This sets the context for how the AI should think about the
                task.
              </p>
            </EditorSurface>

            <EditorSurface title="Analysis instructions">
              <Label className="sr-only" htmlFor="analysis-instructions">
              ...
              />
              <p className="text-sm text-muted-foreground">
                Be specific about what insights you're looking for and how you
                want them presented.
              </p>
            </EditorSurface>
            ...
            <div className="bg-secondary/20 p-4 rounded-md">
              ...
              <ul className="ml-5 list-disc space-y-1">
                <li>
                  Identify common topics that lead to negative customer
                  sentiment and suggest improvements
                </li>
                <li>
                  Analyze response time patterns and their effect on customer
                  satisfaction
                </li>
                <li>
                  Extract the top 5 frequently asked questions from these
                  conversations
                </li>
              </ul>
            </div>
        ...
        >
          <div className="space-y-4">
            ...
            <div className="prose max-w-none">
              ...
              <div className="py-3">
                {' '}
                <div className="markdown-content prose prose-headings:font-semibold prose-headings:text-primary prose-p:text-foreground prose-strong:font-bold prose-strong:text-foreground prose-ul:list-disc prose-ul:pl-6">
